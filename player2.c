...
int main(){
    ...
    while(done==0){
        while(shared_stuff->player_turn==1){//other players turn
            sleep(1);
            if(rand()%20 == 0){
        	    printf("\nWaiting for PLayer 1 to play his move.\n\n");
    	    }
        }
        if(shared_stuff->coins_left==0){//after player 1 move if no more coins left then you lose.
            printf("Sorry! you lost as player1 removed all coins left.\n");
            done=1;
            break;
        }
        ...
        while(valid_input==0 && done==0){
            ...
            printing_gamestate(shared_stuff->gamerows);
            if(shared_stuff->coins_left==0 ){//check if after your move 0 coins left you win and game ends.
                printf("You Won!\n");
                shared_stuff->player_turn=1; 
                done=1;
                break;
            }    
            shared_stuff->player_turn=1;  //after your valid move give turn to player1.
        }
